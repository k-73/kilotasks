...
PlasmaComponents.ContextMenu {
    ...
    PlasmaComponents.MenuItem {
        ...
        visible: visualParent
                     && visualParent.slotIdx === -1
                     && get(atm.IsStartup) !== true
                     && plasmoid.immutability !== PlasmaCore.Types.SystemImmutable

        enabled: visualParent && get(atm.LauncherUrlWithoutIcon) != ""

        text: i18n("Add &slot for this app")
        icon: "list-add"

        onClicked: {
            tasks.addSlotFromTask(visualParent);
        }
    }

    PlasmaComponents.MenuItem {
        visible: visualParent && visualParent.slotIdx >= 0 && visualParent.isEmptySlot === true
        text: i18n("Launch")
        icon: "system-run"
        onClicked: tasks.activateOrSpawnSlot(visualParent.slotIdx, 0)
    }

    PlasmaComponents.MenuItem {
        visible: visualParent && visualParent.slotIdx >= 0
        text: i18n("Duplicate slot")
        icon: "edit-copy"
        onClicked: tasks.duplicateSlot(visualParent.slotIdx)
    }
    ...
    PlasmaComponents.MenuItem {
        ...
        text: i18n("Edit slot…")
        icon: "document-edit"
        onClicked: tasks.editSlotCommand(visualParent)
    }
    ...
    PlasmaComponents.MenuItem {
        visible: visualParent && visualParent.slotIdx >= 0
        text: i18n("Remove slot")
        icon: "window-unpin"
        onClicked: tasks.removeSlot(visualParent.slotIdx)
    }
