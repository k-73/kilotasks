...
// Icon extent the user wants rendered (auto falls back to theme defaults).
function configuredIconExtent() {
    const s = plasmoid.configuration.iconSize || 0;
    if (s > 0) return s;
    return Math.max(
...
// User-configured extra padding added around each icon (outside the theme margins).
function configuredIconPadding() {
    return Math.max(0, plasmoid.configuration.iconPadding || 0);
}

// Full icon cell extent including padding — used as the base for both task cell
// width and height so icon size and padding both actually enlarge the cell.
function paddedIconCellExtent() {
    return configuredIconExtent() + 2 * configuredIconPadding();
}

// Base theme margins only. Extra space now comes from iconGap (real Flow spacing)
// and iconPadding (inside each delegate), exposed as dedicated sliders in the UI.
// Tablet mode keeps its legacy 3x delegate-width boost so touch targets stay usable.
function horizontalMargins() {
    const tabletBoost = Kirigami.Settings.tabletMode ? 3 : 1;
    return (taskFrame.margins.left + taskFrame.margins.right) * (tasks.vertical ? 1 : tabletBoost);
}

function verticalMargins() {
    const tabletBoost = Kirigami.Settings.tabletMode ? 3 : 1;
    return (taskFrame.margins.top + taskFrame.margins.bottom) * (tasks.vertical ? tabletBoost : 1);
}
...
function preferredMaxHeight() {
    if (tasks.vertical) {
      // Vertical panel: icon extent is driven by the panel width. Still honour the
      // configured padding so the cell grows when user bumps the padding slider.
      return verticalMargins() +
             Math.min(
                 tasks.width,
                 tasks.iconsOnly ? tasks.width : paddedIconCellExtent()
             );
    } else {
      // Horizontal panel: cell height = icon extent + padding + theme margins.
      // Previously capped at iconSizes.small * 3 which made the icon-size slider
      // plateau past ~48 px. Drive from configuredIconExtent() instead.
      return verticalMargins() + paddedIconCellExtent();
    }
}
