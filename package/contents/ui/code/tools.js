        ...
        if (task.m.IsLauncher !== true && task.m.IsStartup !== true) {
            if (task.m.IsGroupParent === true) {
                ...
                for (var j = 0; j < tasks.tasksModel.rowCount(modelIndex); ++j) {
                    const childModelIndex = tasks.tasksModel.makeModelIndex(task.taskIdx, j);
    ...
    if (model.IsGroupParent === true) {
        ...
        if (plasmoid.configuration.groupedTaskVisualization === 0) {
            ...
            for (let i = 0; i < tasks.tasksModel.rowCount(task.modelIndex(index)); ++i) {
                const childTaskModelIndex = tasks.tasksModel.makeModelIndex(task.taskIdx, i);
            ...
            if (!childTaskList.some(index => tasks.tasksModel.data(index, TaskManager.AbstractTasksModel.IsActive))) {
                _raiseAndActivate(tasks.tasksModel, lastUsedTask);
            } else {
                ...
                for (let j = 0; j < childTaskList.length; ++j) {
                    const childTask = childTaskList[j];
                        if (tasks.tasksModel.data(childTask, TaskManager.AbstractTasksModel.IsActive)) {
                            ...
                            _raiseAndActivate(tasks.tasksModel, childTaskList[nextTask]);
                            break;
                        }
                }
            }
        ...
        if (model.IsActive === true && plasmoid.configuration.minimizeActiveTaskOnClick) {
            tasks.tasksModel.requestToggleMinimized(index);
        } else {
            // _raiseAndActivate handles IsMinimized / IsHidden unconditionally,
            // which is what users expect from a single left-click on any task
            // (including strays and bound slots whose window is on another
            // desktop / buried under others).
            _raiseAndActivate(tasks.tasksModel, index);
        }
    }
}
